...
function auditTool(entry, context) {
  ...
  const seatCheck = checkSeatOverprovisioning(
    tool,
    planKey,
    seats,
    monthlySpend,
  ...
  const alternative = checkAlternative(
    toolKey,
    planKey,
    seats,
    monthlySpend,
    useCase // ✅ kept useCase usage
  );
  ...
  const bestSaving = recommendations.reduce(
    (max, r) => Math.max(max, r.savingsAmount || 0),
    0
  );
...
function checkDowngrade(tool, planKey, seats) {
  ...
  if (tool.name === "Claude (Anthropic)" && planKey === "team" && seats < 3) {
    ...
    return {
      ...
      reason: `Claude Team is $30/user/month with a 5-seat minimum billing. With ${seats} user(s), individual Pro plans at $20/user save $${teamCost - proCost}/mo with equivalent features for small teams.`,
      savingsAmount: teamCost - proCost,
      recommendedPlan: "pro",
    };
  }
  ...
  if (tool.name === "ChatGPT (OpenAI)" && planKey === "team" && seats === 1) {
    return {
      ...
      reason:
        "Team plan ($30/user) adds an admin console and workspace features that a solo user doesn't need. Plus ($20) gives identical model access.",
      savingsAmount: 10,
  ...
  if (tool.name === "Cursor" && planKey === "business" && seats === 1) {
    return {
      ...
      reason:
        "Business ($40/user) adds centralized billing and an admin dashboard — features irrelevant for a solo user. Pro ($20) has identical AI model access.",
      savingsAmount: 20,
      recommendedPlan: "pro",
    };
  }
...
function checkAlternative(
  ...
  seats,
  monthlySpend,
  useCase // ✅ kept (fixes ESLint)
) {
  if (toolKey === "cursor" && planKey === "pro" && seats >= 3) {
    const savings = 20 * seats - 15 * seats;

    return {
      type: "alternative",
      action: "Consider Windsurf Pro as an alternative",
      reason: `Windsurf Pro at $15/user/month offers comparable AI-assisted coding for teams. Switching ${seats} seats saves $${savings}/mo.`,
      savingsAmount: savings,
  ...
  if (toolKey === "chatgpt" && planKey === "plus" && useCase === "writing") {
    return {
      ...
      reason:
        "Claude Pro ($20/user) is preferred for long-form writing with stronger context handling.",
      savingsAmount: 0,
...
function checkApiVsSeat(
  ...
  if (
    ...
    return {
      ...
      reason: `At $${monthlySpend}/mo on API usage, you're spending at a rate where discounted credits can generate meaningful savings.`,
      savingsAmount: Math.round(monthlySpend * 0.2),
    };
  }
